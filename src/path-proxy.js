...
pathHandler.get = (target, prop) => {
  if (prop in target && isNaN(prop)) {
    ...
    if (childMeta) {
      // api path exists -- update info, stay in this pathProxy FSM state
      let val = _.flow([
...
//   meta: the path structure (JSON Schema with extensions, see path-proxy.spec.js) representing the API endpoints.
